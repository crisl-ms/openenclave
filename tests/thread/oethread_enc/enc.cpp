...
// test_tcs_exhaustion
static size_t g_tcs_used_thread_count = 0;
static oe_spinlock_t g_tcs_lock = OE_SPINLOCK_INITIALIZER;
...
{
    // locking to provide safe synchronous access to g_tcs_used_thread_count
    oe_spin_lock(&g_tcs_lock);
    ++g_tcs_used_thread_count;
    // the local tcs_used_thread_count is used for non-sychronous access
    size_t tcs_used_thread_count = g_tcs_used_thread_count;
    oe_spin_unlock(&g_tcs_lock);
    ...
    // thread count
    while (tcs_used_thread_count + tcs_out_thread_count < tcs_req_count)
    {
        // refresh the local values

        // locking to provide safe synchronous access to g_tcs_used_thread_count
        oe_spin_lock(&g_tcs_lock);
        tcs_used_thread_count = g_tcs_used_thread_count;
        oe_spin_unlock(&g_tcs_lock);
...
size_t enc_tcs_used_thread_count()
{
    // locking to provide safe synchronous access to g_tcs_used_thread_count
    oe_spin_lock(&g_tcs_lock);
    size_t rval = g_tcs_used_thread_count;
    oe_spin_unlock(&g_tcs_lock);
    return rval;
}
