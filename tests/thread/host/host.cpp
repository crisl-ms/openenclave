...
// test_tcs_exhaustion
static std::atomic_flag g_tcs_lock = ATOMIC_FLAG_INIT;
static size_t g_tcs_out_thread_count = 0;
...
void* tcs_thread(oe_enclave_t* enclave, size_t tcs_req_count)
{
    ...
    if (result == OE_OUT_OF_THREADS)
    {
        ...
        // locking to provide safe synchronous access to g_tcs_out_thread_count
        _acquire_lock(&g_tcs_lock);
        ++g_tcs_out_thread_count;
        _release_lock(&g_tcs_lock);
    }
    else
    {
        OE_TEST(result == OE_OK);
...
void test_tcs_exhaustion(oe_enclave_t* enclave)
{
    ...
    // locking to provide safe synchronous access to g_tcs_out_thread_count
    _acquire_lock(&g_tcs_lock);
    // the local tcs_out_thread_count is used for non-synchronous access
    size_t tcs_out_thread_count = g_tcs_out_thread_count;
    _release_lock(&g_tcs_lock);
    ...
    printf(
        ...
        static_cast<unsigned long>(tcs_used_thread_count),
        static_cast<unsigned long>(tcs_out_thread_count));

    // Crux of the test is to get OE_OUT_OF_THREADS i.e. to exhaust the TCSes
    OE_TEST(tcs_out_thread_count > 0);
    // Verifying that everything adds up fine
    OE_TEST(tcs_used_thread_count + tcs_out_thread_count == test_tcs_req_count);
